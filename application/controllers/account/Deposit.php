			...
			$code_method = $this->input->post("method", TRUE);
				...
				if ($code_method == "swift") {
					...
					$transactions = $this->transactions_model->add_transaction(array(
						...
						"label" 	    => $label,
						"admin_comment" 	    => 'none',
						"user_comment" 	    => ''.$merchant_account.'<p><strong> while transfering from Payoneer please do not forget to submit transaction ID in sending Note/Description:  '.$id_transaction.'</strong></p>',
					...
					$transactions = $this->transactions_model->add_transaction(array(
						...
						"label" 	    => $label,
						"admin_comment" 	    => 'none',
						"user_comment" 	    => ''.$merchant_account.'<br><strong> Note for Payoneer Transfer:'.$id_transaction.'</strong>',
						"ip_address" 	    =>  $_SERVER["REMOTE_ADDR"],
						"protect" 	    => "none",
						)
					);
